...
CREATE TABLE `mst_products` (
  ...
  `product_type` enum('SOFTWARE','SERVICE','TRAINING') DEFAULT NULL,
  `description` text,
  `monthly_price` decimal(12,2) DEFAULT NULL,
  `yearly_price` decimal(12,2) DEFAULT NULL,
  `one_time_price` decimal(12,2) DEFAULT NULL,
  `is_active` char(1) DEFAULT 'Y',
  `created_by` bigint DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`product_id`),
  KEY `fk_mst_products_created_by` (`created_by`),
  CONSTRAINT `fk_mst_products_created_by` FOREIGN KEY (`created_by`) REFERENCES `mst_users` (`id`)
...
-- Dump completed on 2026-05-23 11:56:18
